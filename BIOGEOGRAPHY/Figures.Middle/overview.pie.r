...
generate.pie.data<-function(N.country, label, title){
  ...
  plot_data$tab<-label
  gdp_data$tab<-label
  fwrite(plot_data, sprintf("../Figures/BIOGEOGRAPHY/overview.pie.plot_data.%s.csv", label))
  fwrite(gdp_data, sprintf("../Figures/BIOGEOGRAPHY/overview.pie.gdp_data.%s.csv", label))
  p <- ggplot() +
...
p1<-generate.pie.data(N.country, "first.co.author",
                      "First, co-first and corresponding authors")
p1
...
ggsave(final_plot_patchwork, filename="../Figures/BIOGEOGRAPHY/overview.pie.pdf",
       width=13, height=13)
