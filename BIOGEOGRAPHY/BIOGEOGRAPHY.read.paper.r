...
py_run_string("
import requests
def create_request_with_timeout(session, *args, **kwargs):
    kwargs.setdefault('timeout', 1200)
    return super(requests.Session, session).request(*args, **kwargs)
")
s$request <- partial(py$create_request_with_timeout, session = s)
    ...
    tryCatch({
      ...
      if (!is.null(upload.path)){
        ...
        print(system.time({
          ...
          response <- model$generate_content(uploaded_file, stream = TRUE,
                                             request_options = list(timeout = 1200))
          all_chunks <- c()
          cat("--- receiving data from Gemini... ---\n")
          iterate(response, function(chunk) {
            .GlobalEnv$all_chunks <- c(.GlobalEnv$all_chunks, chunk$text)
          })
          full_text <- paste(all_chunks, collapse = "")
          print(full_text)
        ...
        print(system.time({
          ...
          #next()
          response <- model$generate_content(all_text_info, stream = TRUE,
                                             request_options = list(timeout = 1200))
          all_chunks <- c()
          cat("--- receiving data from Gemini... ---\n")
          iterate(response, function(chunk) {
