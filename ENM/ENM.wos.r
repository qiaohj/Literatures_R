...
if (F){
  ...
  #The correct.csv file contains a list of journals, but their names don't appear in the 2024 WOS Core Journal List (jcr.csv). 
  #This could be because the journals were indexed in the past but are no longer, or because their names have changed, 
  #or because the same journals have slightly different names. Please help me find the correct names for all the journals in correct.csv. 
  #If you can't find them, indicate "not in the list." Please display the results in CSV format to facilitate processing in my program.
  
  correct<-fread("../Data/ENM/correct.csv")
  colnames(correct)<-c("Title", "Correct", "ISSN", "eISSN")
  ...
  journals[grepl("&", Title)]
  saveRDS(journals, "../Data/ENM/clean.journal.rda")
  
  
  #TS=("species distribution model*" OR "ecological niche model*" OR "habitat suitability model*" OR "predictive habitat model*" OR "bioclimatic* model*" OR "environmental envelope*" OR "climatic envelope*" OR BIOCLIM)
  ...
  for (i in c(1:nrow(target.journals))){
    ...
    query <- sprintf('TS=("species distribution model*" OR "ecological niche model*" OR "habitat suitability model*" OR "predictive habitat model*" OR "bioclimatic* model*" OR "environmental envelope*" OR "climatic envelope*" OR BIOCLIM) AND (IS=%s)',
                     issn.str)
    database<-"WOS"
    ...
    folder<-sprintf("/media/huijieqiao/WD22T_11/literatures/Data/ENM/WOS/%s", item$Title)
  ...
  for (i in c(1:nrow(target.journals))){
    ...
    saveRDS(all.df, sprintf("../Data/ENM/WOS.RDA/%s.rda", item$Title))
  }
...
target.journals<-readRDS("../Data/ENM/clean.journal.rda")
result<-list()
#target.journals<-target.journals[Title=="AMPHIBIA-REPTILIA"]
i=1
#target.journals<-target.journals[sample(nrow(target.journals), nrow(target.journals))]
no.oa<-c("INTERNATIONAL JOURNAL OF GEOGRAPHICAL INFORMATION SCIENCE", "MARINE ECOLOGY PROGRESS SERIES",
...
for (i in c(1:nrow(target.journals))){
  ...
  if (item$Title %in% no.oa){
    saveRDS(NULL, 
           missing.file)
    next()
  }
  journal.folder<-sprintf("/media/huijieqiao/WD22T_11/literatures/Data/ENM/pdf/%s", item$Title)
  if (!dir.exists(journal.folder)){
    dir.create(journal.folder)
  }
  article.file<-sprintf("../Data/ENM/WOS.RDA/%s.rda", item$Title)
  ...
  articles.crossref$link<-sprintf("https://doi.org/%s", articles.crossref$doi)
  articles.crossref<-articles.crossref[,c("link", "pdf", "uid", "journal", "doi", "title", "url", "resource_primary_url", 
                                          "publisher", "pdf.path", "with.pdf", "doi.prefix", "doi.suffix",
                                          "code", "note", "link")]
...
fwrite(result.df, 
       "/media/huijieqiao/WD22T_11/literatures/Data/ENM/N.papers.csv")
